from Constraints import semanticConstraints_suffix_BMC, consistencyConstraints_suffix, placeholderConstraints, \
     semanticConstraints_cycle_free_suffix_BMC, structureConstraints_cycle_free, \
...
def check_existence_cycle_free_suffix_bmc(sample, sketch):
    t = TicToc()
    t.tic()
    s = Solver()
    ...
    semanticConstraints_suffix_BMC(s, sketch, sample_table, suffix_table, sample.letter2pos)
    ...
    if s.check() == z3.sat:
        ...
        if build_solution:
            build_solution_cycle_free_suffix_bmc(sketch, sample_table, suffix_table, sample, maximumSize)
        if timing:
...
def build_solution_cycle_free_suffix_bmc(sketch, sample_table, suffix_table, sample, finalSize):
    t = TicToc()
    t.tic()
    ...
    semanticConstraints_cycle_free_suffix_BMC(solver_1, sketch_nodes, sample_table, suffix_table, sample.letter2pos)
                        ...
                        if k < len(trace) - 1:
                            solver_1.add(
                                Implies(
                                    ...
                                    Or(
                                        Bool('z_%s_%s_%s' % (leftid, s, k)),
                                        Bool('z_%s_%s_%s' % (new_node_id, s, k + 1))
                                    )
                                )
                            )
                        else:   # k == len(trace) - 1
