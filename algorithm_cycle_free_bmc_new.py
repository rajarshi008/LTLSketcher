from z3 import *
from Constraints import semanticConstraints_BMC, semanticConstraints_cycle_free_BMC, consistencyConstraints, \
...
def check_existence_cycle_free_bmc(sample, sketch):
    t = TicToc()
    t.tic()
    s = Solver()
    ...
    if s.check() == z3.sat:
        ...
        if build_solution:
            build_solution_cycle_free_bmc(sketch, sample, maximumSize)
        if timing:
...
def build_solution_cycle_free_bmc(sketch, sample, finalSize):
    t = TicToc()
    t.tic()
    ...
    semanticConstraints_cycle_free_BMC(solver_1, sketch_nodes, sample)
                        ...
                        if k < trace.length - 1:
                            solver_1.add(
                                Implies(
                                    ...
                                    Or(
                                        Bool('z_%s_%s_%s' % (leftid, j, k)),
                                        Bool('z_%s_%s_%s' % (new_node_id, j, k + 1))
                                    )
                                )
                            )
                        else:   # k == trace.length - 1
